...
function pendingHarness(volume = 10, playbackResults = [false, true]) {
  ...
  vm.runInNewContext(`
    ...
    this.api = {
      ...
      retry() {
        agentSoundRetryPendingTerminal();
      },
      setVolume(value) {
      ...
      pending() {
        return agentSoundPendingTerminal ? { ...agentSoundPendingTerminal } : null;
      }
    };
  `, context);
...
test('unavailable terminal playback is retained and later replayed exactly once', () => {
  ...
  assert.equal(harness.api.pending()?.kind, 'success',
  ...
  assert.equal(harness.api.pending(), null, 'successful retry must consume the pending cue');
...
test('later terminal state replaces an older undelivered cue without creating a queue', () => {
  const harness = pendingHarness(10, [false, false, true]);
  ...
  assert.equal(harness.api.pending()?.kind, 'error',
    'newer error terminal must replace the older undelivered success cue');

  harness.api.retry();
  assert.deepEqual(harness.emitted, ['success', 'error', 'error'],
    'unlock must replay only the single newest pending terminal cue');
  assert.equal(harness.api.pending(), null);
  assert.equal(harness.api.keys().length, 1,
    'only the cue that actually started playback may be marked delivered');
});
...
test('volume zero suppresses new pending cues and clears retained cues before replay', () => {
  ...
  assert.equal(mutedAfterTerminal.api.pending(), null,
    'volume zero must discard the retained terminal cue');
...
test('successful audio unlock owns pending replay instead of requiring terminal re-observation', () => {
  ...
  assert.match(unlock, /agentSoundRetryPendingTerminal\(\)/,
