...
text = path.read_text()
assert '// @version      0.6.121' in text
text = text.replace('// @version      0.6.121', '// @version      0.6.122', 1)

old = "  let testInProgress = false;\n  let diagnosticLog = [];"
new = "  let testInProgress = false;\n  let jumpInProgress = false;\n  let diagnosticLog = [];"
assert old in text
text = text.replace(old, new, 1)

marker = "  async function runExport(kind) {"
assert marker in text
jump_code = r'''  function jumpUserRecords(spine) {
...
'''
text = text.replace(marker, jump_code + marker, 1)
text = text.replace(
...
test_code = r'''  function testJumpIdentifierResolution() {
    const records = [
...
text = text.replace(needle, needle + "      await run('Jump identifier resolution', testJumpIdentifierResolution);\n", 1)

old_ui = "    const test = panel.querySelector('[data-role=\"test\"]');\n"
assert old_ui in text
text = text.replace(old_ui, old_ui + "    const jump = panel.querySelector('[data-role=\"jump\"]');\n", 1)
text = text.replace("md.disabled = exportInProgress || testInProgress;", "md.disabled = exportInProgress || testInProgress || jumpInProgress;", 1)
text = text.replace("jsonl.disabled = exportInProgress || testInProgress;", "jsonl.disabled = exportInProgress || testInProgress || jumpInProgress;", 1)
text = text.replace("test.disabled = exportInProgress || testInProgress;", "test.disabled = exportInProgress || testInProgress || jumpInProgress;", 1)
ui_needle = "    if (test) {\n      test.disabled = exportInProgress || testInProgress || jumpInProgress;\n      test.textContent = testInProgress ? 'Testing…' : 'Test';\n    }\n"
assert ui_needle in text
text = text.replace(ui_needle, ui_needle + "    if (jump) {\n      jump.disabled = exportInProgress || testInProgress || jumpInProgress;\n      jump.textContent = jumpInProgress ? 'Jumping…' : 'Jump';\n    }\n", 1)

panel_needle = '      <div class="tm-row"><button data-role="extract-jsonl" type="button">Extract JSONL</button><button data-role="extract-md" type="button">Extract MD</button></div>\n'
assert panel_needle in text
text = text.replace(panel_needle, '      <div class="tm-row"><button data-role="jump" type="button">Jump</button></div>\n' + panel_needle, 1)
listener_needle = "    panel.querySelector('[data-role=\"test\"]').addEventListener('click', () => void runTests());\n"
assert listener_needle in text
text = text.replace(listener_needle, listener_needle + "    panel.querySelector('[data-role=\"jump\"]').addEventListener('click', () => void runJump());\n", 1)
