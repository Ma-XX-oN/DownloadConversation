...
text = replace_once(text, "    const copyLogButton = panel.querySelector('[data-role=\"copy-log\"]');", "    if (consoleOutput) {\n      consoleOutput.checked = consoleDiagnostics;\n      consoleOutput.addEventListener('change', () => {\n        consoleDiagnostics = consoleOutput.checked;\n        localStorage.setItem(CONSOLE_DIAGNOSTICS_STORAGE_KEY, String(consoleDiagnostics));\n        logDiagnostic('debug', 'console-diagnostics-changed', { enabled: consoleDiagnostics });\n      });\n    }\n    const copyLogButton = panel.querySelector('[data-role=\"copy-log\"]');", 'listener')
text = replace_once(text, "  function makePanel() {\n    if (document.getElementById(PANEL_ID) || !document.body) return;", "  function makePanel() {\n    if (document.getElementById(PANEL_ID) || !document.body) return;\n    logDiagnostic('verbose', 'recorder-panel-create-start', { body_connected: document.body.isConnected, diagnostics_level: diagnosticsLevel });", 'panel lifecycle')
text = replace_once(text, "  function bootstrapUi() {\n    if (document.body) {", "  function bootstrapUi() {\n    logDiagnostic('verbose', 'recorder-bootstrap-ui', { body_present: Boolean(document.body), ready_state: document.readyState, diagnostics_level: diagnosticsLevel });\n    if (document.body) {", 'bootstrap lifecycle')
path.write_text(text, encoding='utf-8')

panel_path = root / 'tests/recorder-panel-ui.test.mjs'
panel = panel_path.read_text(encoding='utf-8')
panel = replace_once(panel, "  assert.match(userscript, /data-role=\"show-turn-ids\" type=\"checkbox\"> Turn ID/);", "  assert.match(userscript, /data-role=\"show-turn-ids\" type=\"checkbox\"> Turn ID/);\n  assert.match(userscript, /data-role=\"console-diagnostics\" type=\"checkbox\"> Console/);\n  assert.match(userscript, /CONSOLE_DIAGNOSTICS_STORAGE_KEY/);", 'panel test')
...
test('console diagnostics precede panel threshold filtering', () => {
  assert.match(userscript, /\/\/ @version      0\\.6\\.153/);
  const start = userscript.indexOf('  function logDiagnostic(');
  const end = userscript.indexOf('  function persistDiagnosticLog', start);
  const body = userscript.slice(start, end);
...
test('startup lifecycle includes diagnostics preference', () => {
  assert.match(userscript, /recorder-bootstrap-ui/);
  assert.match(userscript, /recorder-panel-create-start/);
  assert.match(userscript, /diagnostics_level: diagnosticsLevel/);
});
""", encoding='utf-8')
