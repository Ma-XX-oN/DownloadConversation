...
old_design = "design_path.write_text(design.rstrip() + addition + '\\n', encoding='utf-8')"
new_design = "design_path.write_text(design.rstrip() + addition.rstrip() + '\\n', encoding='utf-8')"
if old_design not in text:
  raise SystemExit('design trailing-newline write not found in helper')
path.write_text(text.replace(old_design, new_design, 1), encoding='utf-8')
