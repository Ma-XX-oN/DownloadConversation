...
## Shared agent lifecycle observation and terminal dispatch
...
## Modular userscript source and deterministic build

Issue #150 makes `src/userscript-header.js`, `src/userscript-manifest.json`, and the ordered files under `src/userscript/` the authoritative DownloadConversation source. The root `chatgpt-conversation-markdown-export.user.js` is generated output and is not edited directly. The manifest groups ordered source segments under logical subsystem ownership while preserving the original userscript IIFE execution order; segmentation is a source/build layout boundary, not a runtime module loader or a semantic rewrite.

The build pins AIConversationCore by exact repository commit, Git blob SHA-1, and byte length. `scripts/build-userscript.mjs` fetches that exact classic-script artifact, verifies its bytes before assembly, places the verified Core bundle before the DownloadConversation IIFE, and emits one readable Tampermonkey userscript. The generated file contains no userscript `@require` directive, so Core loading no longer introduces a runtime dependency wait. Core and DownloadConversation retain separate version and provenance identities.

Build determinism means the same authoritative DownloadConversation source plus the same verified pinned dependency bytes produces the same generated userscript bytes. `node scripts/build-userscript.mjs --check` rejects stale generated output. CI builds the production artifact before syntax, JSDoc, and focused regressions so tests that inspect production code consume the generated artifact rather than the retired monolith.

The pre-#150 monolithic userscript remains only as a migration/provenance fixture with its original Git blob identity. It is not authoritative source and must not be edited to make production changes. Migration regression coverage proves that the ordered authoritative source retains every non-whitespace legacy runtime byte in order; the already-established transfer-boundary whitespace differences are not runtime feature changes. No fallback runtime loading path is introduced by this build layout.
